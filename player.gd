...
@onready var game_over_ui = get_tree().get_root().find_child("GameOverUI", true, false)
@onready var win_ui = get_tree().get_root().find_child("WinUI", true, false)
...
@onready var restart_button = get_tree().get_root().find_child("RestartButton", true, false)  
...
func _ready():
	...
	if game_timer:
		...
		game_timer.wait_time = 50  # Ensure it uses the correct time
		game_timer.start()
